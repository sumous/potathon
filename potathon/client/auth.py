...
class AuthMethods:
    ...
    async def send_code_captcha_request(self, phone, force_sms=False):
        result = None
        phone = utils.parse_phone(phone) or self._phone
        ...
        if not phone_hash:
            try:
                result = await self(functions.auth.SendCodeCaptchaRequest(
                    phone, self.api_id, self.api_hash))
            except errors.AuthRestartError:
                return await self.send_code_request(phone, force_sms=force_sms)

            if isinstance(result, types.auth.SentCodeCaptcha):
                return result
        ...
        if force_sms:
            result = await self(
                functions.auth.ResendCodeCaptchaRequest(phone, phone_hash))
