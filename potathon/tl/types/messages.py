...
if TYPE_CHECKING:
    from ...tl.types import TypeBotInlineResult, TypeChat, TypeChatFull, TypeDialog, TypeDocument, TypeEncryptedFile, TypeHighScore, TypeInlineBotSwitchPM, TypeMessage, TypeMessageUserVote, TypeMessagesFilter, TypeStickerPack, TypeStickerSet, TypeStickerSetCovered, TypeUser
...
class ChannelMessages(TLObject):
    CONSTRUCTOR_ID = 0xf6777771
    SUBCLASS_OF_ID = 0xd4b40b5e

    def __init__(self, count: int, messages: List['TypeMessage'], chats: List['TypeChat'], users: List['TypeUser'], inexact: Optional[bool]=None, next_rate: Optional[datetime]=None):
        """
        Constructor for messages.Messages: Instance of either Messages, MessagesSlice, ChannelMessages, ChannelOldMessages, MessagesNotModified.
        """
        self.count = count
    ...
    def _bytes(self):
        return b''.join((
            b'qww\xf6',
            struct.pack('<I', (0 if self.inexact is None or self.inexact is False else 2) | (0 if self.next_rate is None or self.next_rate is False else 1)),
            struct.pack('<i', self.count),
            b'' if self.next_rate is None or self.next_rate is False else (self.serialize_datetime(self.next_rate)),
    ...
    def from_reader(cls, reader):
        ...
        return cls(count=_count, messages=_messages, chats=_chats, users=_users, inexact=_inexact, next_rate=_next_rate)


class ChannelOldMessages(TLObject):
    CONSTRUCTOR_ID = 0x99262e37
    SUBCLASS_OF_ID = 0xd4b40b5e

    def __init__(self, pts: int, count: int, messages: List['TypeMessage'], chats: List['TypeChat'], users: List['TypeUser']):
        """
        Constructor for messages.Messages: Instance of either Messages, MessagesSlice, ChannelMessages, ChannelOldMessages, MessagesNotModified.
        """
        self.pts = pts
...
class MessagesSlice(TLObject):
    ...
    def __init__(self, count: int, messages: List['TypeMessage'], chats: List['TypeChat'], users: List['TypeUser']):
        """
        Constructor for messages.Messages: Instance of either Messages, MessagesSlice, ChannelMessages, ChannelOldMessages, MessagesNotModified.
        """
        self.count = count
