...
if TYPE_CHECKING:
    ...
    from ...tl.types import TypeCdnFileHash
...
class FileCdnRedirect(TLObject):
    ...
    def __init__(self, dc_id: int, file_token: bytes, encryption_key: bytes, encryption_iv: bytes, cdn_file_hashes: List['TypeCdnFileHash']):
        """
        Constructor for upload.File: Instance of either File, FileCdnRedirect.
        """
        self.dc_id = dc_id
    ...
    def to_dict(self):
        return {
            ...
            'cdn_file_hashes': [] if self.cdn_file_hashes is None else [x.to_dict() if isinstance(x, TLObject) else x for x in self.cdn_file_hashes]
        }
    ...
    def _bytes(self):
        return b''.join((
            ...
            b'\x15\xc4\xb5\x1c',struct.pack('<i', len(self.cdn_file_hashes)),b''.join(x._bytes() for x in self.cdn_file_hashes),
        ))
    ...
    def from_reader(cls, reader):
        ...
        return cls(dc_id=_dc_id, file_token=_file_token, encryption_key=_encryption_key, encryption_iv=_encryption_iv, cdn_file_hashes=_cdn_file_hashes)
