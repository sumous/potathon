...
if TYPE_CHECKING:
    from ...tl.types import TypeChannelAdminLogEventsFilter, TypeChannelParticipantsFilter, TypeChatAdminRights, TypeChatBannedRights, TypeInputChannel, TypeInputChatPhoto, TypeInputCheckPasswordSRP, TypeInputGeoPoint, TypeInputStickerSet, TypeInputUser
...
class CreateChannelRequest(TLRequest):
    ...
    def __init__(self, title: str, about: str, broadcast: Optional[bool]=None, megagroup: Optional[bool]=None, geo_point: Optional['TypeInputGeoPoint']=None, address: Optional[str]=None):
        """
    ...
    def _bytes(self):
        ...
        return b''.join((
            b'\x0f\xb1_=',
            struct.pack('<I', (0 if self.broadcast is None or self.broadcast is False else 1) | (0 if self.megagroup is None or self.megagroup is False else 2) | (0 if self.geo_point is None or self.geo_point is False else 4) | (0 if self.address is None or self.address is False else 4)),
    ...
    def from_reader(cls, reader):
        ...
        if flags & 4:
            ...
            _address = None
        return cls(title=_title, about=_about, broadcast=_broadcast, megagroup=_megagroup, geo_point=_geo_point, address=_address)
...
class GetFullChannelRequest(TLRequest):
    CONSTRUCTOR_ID = 0xb2e02779
    SUBCLASS_OF_ID = 0x225a5109

    def __init__(self, channel: 'TypeInputChannel', reserve: int):
        """
    ...
    def _bytes(self):
        return b''.join((
            b"y'\xe0\xb2",
            self.channel._bytes(),
            struct.pack('<i', self.reserve),
        ))

    @classmethod
    def from_reader(cls, reader):
        _channel = reader.tgread_object()
        _reserve = reader.read_int()
        return cls(channel=_channel, reserve=_reserve)
