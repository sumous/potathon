...
GENERATOR_DIR = Path('potathon_generator')
LIBRARY_DIR = Path('potathon')
...
def generate(which, action='gen'):
    from potathon_generator.parsers import\
        parse_errors, parse_methods, parse_tl, find_layer

    from potathon_generator.generators import\
...
def main():
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        for x in ('build', 'dist', 'Potathon.egg-info'):
        ...
        with open('potathon/version.py', 'r', encoding='utf-8') as f:
            version = re.search(r"^__version__\s*=\s*'(.*)'.*$",
                                f.read(), flags=re.MULTILINE).group(1)
        setup(
            name='Potathon',
            version=version,
            description="Full-featured potato client library for Python 3",
            ...
            keywords='potato api chat client library messaging mtproto',
            packages=find_packages(exclude=[
                'potathon_*', 'run_tests.py', 'try_potathon.py'
            ]),
